...
=head1 SYNOPSIS
 ...
 int main(void) {
     ...
     int records = 100;
     char *name = "test";
     int recsize = sizeof(stuff_t);

     db = rms_create(path, name, records, recsize, retries, timeout, &dump);

     stat = rms_open(db);
     stat = rms_get(db, recno, &stuff);
     stat = rms_put(db, recno, &stuff);
     stat = rms_close(db);
...
=head2 I<int rms_add(rms_t *self, void *data)>
...
=head2 I<int rms_find(rms_t *self, void *data, int len,  int (*compare)(void *, void *), int *recnum)>
...
=head2 I<int rms_search(rms_t *self, void *data, int len, int (*compare)(void *, void *), int (*capture)(void *, error_trace_t *))>
