...
The database consists of these three files:
 ...
 profiles.dat - where the optional user profiles are stored
...
=head2 I<int user_find(user_t *self, void *data, int len,  int (*compare)(void *, int, user_base_t *), int *index)>
...
=head2 I<int user_search(user_t *self, void *data, int len, int (*compare)(void *, int, user_base_t *), queue *results)>
