...
limited time it needs to read and write data. This should minimize the
...
of the data. It will take a lock and compare the value in the header
...
the same. It also make use of the L<errors(3)> and L<tracer(3)>  
...
The base message number. 

=item B<dump>

A pointer to the data structure that is used to manage a trace dump.
...
=head2 I<int jam_search_messages(jam_t *self, ulong select, int index, int (*compare)(ulong, jam_message_t *), queue *results)>
...
A Boolean on wither to just use the index or to also include the message body
in the search.
