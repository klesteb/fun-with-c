...
int main(int argc, char **argv) {
    ...
    if ((window = window_create(row, col, height, width))) {
        ...
        if ((vertical = vertical_create(1, 1, height - 2, width - 2)) == NULL) {
        ...
        if ((stat = container_add_component(vertical, label)) != OK) {
            ...
            printw("unable to attach label to container\n");
            goto fini;
        ...
        if ((stat = container_add_component(vertical, hline)) != OK) {
            ...
            printw("unable to attach hline to container\n");
            goto fini;
