...
int main(int argc, char **argv) {
    ...
    if ((window = window_create(row, col, height, width))) {
        ...
        if ((vertical = vertical_create(2, 1, 1, 15)) == NULL) {

            printw("unable to create vertical container\n");
            goto fini;
        ...
        if ((stat = container_add_component(vertical, hline)) != OK) {

            printw("unable to attach vline to vertical container\n");
            goto fini;
        ...
        if ((stat = window_add_container(window, vertical)) != OK) {
            ...
            printw("unable to attach vertical container to window\n");
            goto fini;
