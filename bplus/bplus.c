/********************************************************************/
/*                                                                  */
/*             BPLUS file indexing program - Version 1.2            */
/*                                                                  */
/*                      A "shareware program"                       */
/*                                                                  */
/*                                                                  */
...
void  write_if(handle, start, buf, nwrt)
  int handle;
  long start;
  char *buf;
  int nwrt;
...
int  find_ix(pe, pix, find)
  ...
  int find;
  {
    ...
    while (ads != NULLREC)
     ...
     if (h && find)			/* change added 4-20-89 */
     {
       if (!ret)
       {
...
int  find_exact(pe, pix)
  ...
  IX_DESC * pix;
  {
    ...
    if ( ret && pci->duplicate)
      {
		while (e.recptr != pe->recptr)			   	/* changes 4-14-89 */
			{
			if (next_key(&e,pci) == EOIX)
				{
				return(0);		/* no more records */
				}
			if (strcmp(e.key,pe->key) != 0)
				{
				return(0);		/* keys changed! */
				}
			}
      }
    copy_entry(pe, &e);
