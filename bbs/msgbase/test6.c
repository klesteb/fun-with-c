...
int main(int argc, char **argv) {
    ...
    when_error_in {
        ...
        stat = jam_new_field(jam, JAMSFLD_SENDERNAME, sysop, &field1);
        check_return(stat, jam);

        stat = que_push_tail(&fields, field1);
        check_status(stat, QUE_OK, E_INVOPS);

        stat = jam_new_field(jam, JAMSFLD_RECVRNAME, to, &field2);
        check_return(stat, jam);

        stat = que_push_tail(&fields, field2);
        check_status(stat, QUE_OK, E_INVOPS);

        stat = jam_new_field(jam, JAMSFLD_SUBJECT, subject, &field3);
        check_return(stat, jam);
